...
bool I2CMaster::sendParam(uint8_t slave_adress, uint8_t mode, float kp, float ki, float kd) {
    ...
    uint32_t scaled_kp = kp*1000;   //Max 65.535 (Can contain three decimals)
    uint32_t scaled_ki = ki*800;    //Max 81.918 (Can contain three decimals)
    uint32_t scaled_kd = kd*10000;  //Max 6.5535 (Can contain four decimals)

    Wire.beginTransmission(slave_adress);
    Wire.write(CMD_PARAM);
    Wire.write(mode);
    Wire.write(byte((scaled_kp >> 16) & 0xFF));
    Wire.write(byte((scaled_kp >> 8) & 0xFF));
    Wire.write(byte(scaled_kp & 0xFF));
    Wire.write(byte((scaled_ki >> 16) & 0xFF));
    Wire.write(byte((scaled_ki >> 8) & 0xFF));
    Wire.write(byte(scaled_ki & 0xFF));
    Wire.write(byte((scaled_kd >> 16) & 0xFF));
